...
def _daily_system_message(asset_label: str) -> str:
    return (
...
def _intraday_system_message(asset_label: str) -> str:
    return (
        ...
        f"Use a SHORT lookback: call get_news(query, start_date, end_date) with start_date and end_date covering only the last 1 day, and get_global_news(curr_date, look_back_days, limit) with look_back_days=1 for the freshest broad headlines. Use get_macro_indicators only if a macro print is hitting RIGHT NOW and could spike intraday volatility — otherwise de-emphasize it. Use get_prediction_markets(topic, limit) when a live event (e.g. an imminent decision, listing vote, or rate decision) has a near-term resolution that could jolt price in the next couple of hours. "
...
def create_news_analyst(llm):
    def news_analyst_node(state):
        ...
        base_message = (
            _intraday_system_message(asset_label)
            if intraday
            else _daily_system_message(asset_label)
        )
        system_message = (
            base_message
